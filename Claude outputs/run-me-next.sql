-- =====================================================================
-- Untangled Life — run this in the Supabase SQL editor.
--
-- Everything added on 14 September 2026, in order, plus two files you
-- have already run that have since been corrected. Safe to re-run in
-- full: every policy and constraint is dropped before it is created.
--
--   0a. photos.sql            RE-RUN — the version you ran used
--                             `on conflict do nothing`, which would
--                             have left an existing bucket public.
--   0b. leaving.sql           RE-RUN — review found a concurrent-unpair
--                             hole and an account deletion that could
...
--   4.  free-time-prefs.sql   what counts as time you're free
--   5.  home-layout.sql       per-person Home screen arrangement
--
-- The one thing that is not idempotent in spirit is the
-- `delete from busy_blocks` in calendar-sharing — it clears the table so
-- no row written under the old sharing rule outlives it. Re-running
-- costs one calendar refresh, nothing more.
-- =====================================================================
